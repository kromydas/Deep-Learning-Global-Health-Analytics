...
#     """
#     Crop raster based on provided coordinates and saves the resulting tiles into the specified folder. The function
...
#         a 'cluster_id' key. The 'point' should have x (longitude) and y (latitude) coordinates in the projected CRS.
...
#         # print(f"cluster_id: {cluster_id}, type: {type(cluster_id)}")
#
...
#         window = rasterio.windows.Window(px_ul_x, px_ul_y, tile_size, tile_size)
#         # Ensure window is within the raster bounds
#         window = window.intersection(rasterio.windows.Window(0, 0, rasterio_dataset.width, rasterio_dataset.height))
#
...
#                     'w',
#                     driver='GTiff',
#                     height=window.height,  # Use window's height
#                     width=window.width,  # Use window's width
#                     count=rasterio_dataset.count,
#                     dtype=rasterio_dataset.dtypes[0],
#                     crs=rasterio_dataset.crs,
#                     transform=transform
...
#                 dst.write(rasterio_dataset.read(window=window, boundless=True, fill_value=0))
#         else:
#             print(f"No valid data for point at index {idx} with cluster ID {cluster_id}")
#
#     print(f"Crops are saved in {output_folder}")
...
#         data = src.read(1)  # Assuming a single-band raster; adjust if multi-band
#         profile = src.profile
#
#         # Apply NoData replacement logic
#         if src_nodata is not None:
#             # Identify where the source data is equal to the specified NoData value
#             nodata_mask = (data == src_nodata)
#             # Replace NoData in the source data with the destination NoData value
#             data[nodata_mask] = dst_nodata
#         else:
#             # If no src_nodata provided, assume handling based on existing NoData
#             nodata_mask = src.nodata
#             data[data == nodata_mask] = dst_nodata
#
#         # Update the raster profile to specify the new NoData value
#         profile.update(nodata=dst_nodata)
#
#         if debug:
#             print(f"Updated raster profile nodata to {dst_nodata}\n")
#
#         # Save the modified raster data
#         with rasterio.open(output_tif, 'w', **profile) as dst:
#             dst.write(data, 1)  # Writing data to the first band
#
#     if debug:
#         print(f"Output TIF has been written to: {output_tif}\n")
...
#
#             mask_no_data = data == no_data      # Boolean mask to identify no_data pixels
#             mask_bad_value = data == bad_value  # Boolean mask to identify bad_value pixels
#
#             # Extracts pixels that are not flagged as NoData or bad values
...
#                 norm_data[mask_no_data] = np.nan  # Treat NoData by setting it to NaN
#                 norm_data[mask_bad_value] = -1  # Mark bad values distinctly
#
#                 plt.figure(figsize=(8, 6))
#                 cmap_modified = plt.get_cmap(cmap)
#                 colors = cmap_modified(np.linspace(0, 1, 256))
#                 colors = np.vstack(([1, 0, 1, 1], colors))  # Add magenta at the start for bad values
#                 cmap_modified = Normalize(vmin=-1, vmax=1)
...
# def load_and_plot_geotiffs(file_path, plot_data=True, cmap='gray', precision=np.float32, plot_geo_coords=False, bad_value=-9999):
#     files = glob(file_path)
...
#             #     min_val = np.nanpercentile(valid_data, 0.1)
#             #     max_val = np.nanpercentile(valid_data, 99.9)
#             # else:
#             #     min_val = 0
#             #     max_val = 1
#
...
#                 norm_valid_data = np.clip((valid_data - min_val) / (max_val - min_val), 0, 1)
#
#                 # Create a normalized data array with the same shape as the original
#                 norm_data = np.full_like(data, np.nan, dtype=precision)
#                 norm_data[~mask_no_data & ~mask_bad_value] = norm_valid_data
#
...
#
#             # Compute statistics from valid data
#             no_data_count = np.sum(mask_no_data)
#             bad_value_count = np.sum(mask_bad_value)
#             total_pixels = data.size
#             no_data_percent = (no_data_count / total_pixels) * 100
